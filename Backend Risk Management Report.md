...
***
## 1. 개요 (Overview)
본 문서는 Chrome Extension Manifest V3 환경에서 Anywhere Music을 개발하며 발생한 주요 기술적 난관과 이를 해결하기 위한 과정을 기술했습니다.

## 2. 리스크 및 해결 과정
### 1. 팝업 종료 시 프로세스 소멸.
> 발생 상황
>* 사용자가 확장 프로그램 팝업(UI)을 닫으면, 팝업 내에서 실행되던 JavaScript도 즉시 종료.

> 해결
>* Message Passing & Storage 패턴 적용.
>* 책임 전가: 오디오 재생의 주체를 팝업이 아닌 백그라운드와 오디오 엔진으로 이관.
>* 상태 보존: 팝업은 단순히 "명령"만 내리고 소멸됨. 백그라운드는 이 명령을 수행하고, 현재 상태를 chrome.storage.local에 저장하여 팝업이 다시 열릴 때 상태를 복구하도록 구현.


### 2. 서비스 워커 비활성화.
> 발생 상황
>* Manifest V3의 Service Worker는 일정 시간(약 30초~5분) 동안 활동이 없으면 배터리 절약을 위해 '수면 모드'로 전환됨. 이때 메모리에 저장된 변수가 초기화.

> 해결
>* Storage First 전략.
>* 중요한 상태(재생 목록, 인덱스)가 변경될 때마다 즉시 chrome.storage.local에 동기화.
>* 서비스 워커가 깨어날 때, restoreState() 함수가 가장 먼저 실행되어 디스크에서 메모리로 상태를 복구함으로써 사용자 경험의 단절을 방지.
...
> #### 2차 시도.
>* Manifest.json의 오디오 권한 확대.
>> #### 결과: 실패.
>>* 확장 프로그램 추가 단계에서 크롬의 거부 반응 발생.
>>* Chrome Web Store 정책상 불필요하게 넓은 권한 요청은 심사 거절 사유가 되며, V3에서는 보안상 오디오 권한 확장이 엄격히 제한됨.
>>* 결국 시작도 못 함.
...
>* Sandbox 형식 삭제 및 Raw Iframe으로 대체.
>* player.html 및 player.js 삭제. 역할을 다시 Offscreen으로 이관 및 manifest에서 sandbox 파라미터 제거.
>> #### 결과: 실패.
>>* 여전히 오디오 재생이 안 됨(아오).
...
>* 크롬은 사용자가 직접 클릭하지 않은 프레임(Offscreen)에서 소리가 켜진 채로 시작하는 걸 극도로 싫어함.
>* 음소거(Mute) 상태로 몰래 재생을 시작한 뒤, 영상이 돌기 시작하면 1초 뒤에 슬쩍 소리를 켜기.
>> #### 결과: 실패.
>>* 여전히 오디오 안 나옴.
...
>* 검색할 때 퍼가기가 허용된 영상만 로드시키는 필터 추가.
>> #### 결과: 실패.
>>* 여전히 오디오 안 나옴.
...
>* Offscreen 문서 자체에 Web Audio API로 '삐-' 소리를 억지로 만들어내서 오디오 엔진을 강제로 깨우기.
>> #### 결과: 실패.
>>* 여전히 오디오 안 나옴.
...
>* 한 번만 명령하고 마는 게 아니라, 플레이어가 재생 응답을 할 때까지 0.5초마다 계속 재생하라는 메시지를 보내는 로직으로 수정.
>> #### 결과: 실패.
>>* 여전히 오디오 안 나옴.
...
>* 플레이어를 매번 죽이지 않고 딱 한 번만 만들어서 계속 재사용하는 구조로 변경. 또한 영상 로딩 방식을 src 속성이 아닌, 훨씬 강력한 자바스크립트 명령(loadVideoById)으로 변경.
>> #### 결과: 실패.
>>* 여전히 오디오 안 나옴.
...
>* 도메인 변경(www.youtube.com -> www.youtube-nocookie.com) 및 파라미터 제거(origin, widget_referrer)를 통해 보안 검사 우회. manifest.json 업데이트.
>> #### 결과: 실패.
>>* 여전히 오디오 안 나옴.
...
>* 출처(origin)를 밝히지 않고, 파라미터를 최소화해서 익명으로 접속해 보안 검사 우회 시도.
>> #### 결과: 실패.
>>* 여전히 오디오 안 나옴.
>>* 기존 방식으로 다시 회귀.
...
>* 재생이 되지만, 곧바로 차단하는 걸 수도 있음. 이번에는 재생 중이라도 계속 요청을 보내도록 수정.
>> #### 결과: 실패.
>>* 여전히 오디오 안 나옴.
